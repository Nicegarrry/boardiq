...
function SectionDivider({ label }: { label: string }) {
  return (
    <div className="mt-5 mb-3 flex items-center gap-3">
      <p className="text-[10px] uppercase tracking-[1.5px] font-bold text-ink-muted shrink-0">
        {label}
      </p>
      ...
      <div className="flex-1 overflow-y-auto px-5 py-4">
        ...
        {(isPresenter || isVoter) && (
          <div
            className={
              isPresenter
                ? 'bg-ember-tint border border-ember-border rounded-md p-3 mb-3'
                : 'bg-paper-dark border border-border-main rounded-md p-3 mb-3'
            }
          >
        ...
        {item.iqAnalysis && (
          <>
            <SectionDivider label="IQ Analysis" />
            <IQPanel itemId={item.id} userId={userId} inline />
          </>
        )}
        ...
        {item.documents.length > 0 && (
          <>
            <SectionDivider label={`Documents (${item.documents.length})`} />
            <div>
